...
data = pd.read_excel('data/brute/produits_uber_eats.xlsx')
data = delete_null_values(data, 'product_name')

data['product_name_final'] = data['product_name']
...
data.to_csv(path_or_buf='data/cleaned/produits_uber_eats_cleaned.csv', sep=';', index=False)
